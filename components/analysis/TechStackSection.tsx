...
export function TechnologyMark({ technology }: { technology: Technology }) {
  return (
    <span
      className={`technology-mark technology-${technology.color}`}
      aria-hidden="true"
    >
      {technology.mark}
    </span>
  ...
  return (
    <section>
      ...
      <p className="section-description">
        A modern web stack, with a clear role for every piece.
      </p>
        ...
        {technologies.map((technology) => (
          <article className="panel-card technology-card" key={technology.name}>
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h3 className="small-heading">{technology.name}</h3>
                <p className="mt-1 text-xs text-muted">{technology.category}</p>
              </div>
            </div>
